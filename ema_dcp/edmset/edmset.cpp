// edmset.cpp : Defines the entry point for the console application.
//
...
int SGmanualGet(char *ip, int16_t portid, uint32_t gtype, uint32_t id, uint32_t tbl_id, int blDebugMode)
{
	int32_t err = 0 ;
	LOG_HEADER Logger;
    COMM_PORT *cp = NULL;
    ...
    REM_ADDR raddr;
	DCP_REG rparam[1];
	DCP_ASTRUCT_MNL *rbufA = NULL ;
	DCP_BSTRUCT_MNL *rbufB = NULL ;
    ...
    {
        if (blDebugMode==1) printf("RSDURTGUtils_GetCommPort() failed, err %04hx\n", status);
		printf("NONE");
    }
    else
    {
        msgheader.command = CMD_DCP_GETVAL;
        ...
        msgheader.data_len = sizeof(DCP_REG)*1;

        rparam[0].tbl_id = 0; // tbl_id
		rparam[0].id = id;
        rparam[0].status = DCP_OK;
		  
		rbufA =(DCP_ASTRUCT_MNL *) RSDURTGUtils_SMalloc(1*sizeof(DCP_ASTRUCT_MNL));
		rbufB =(DCP_BSTRUCT_MNL *) RSDURTGUtils_SMalloc(1*sizeof(DCP_BSTRUCT_MNL));
			...
			for(uint32_t i=0; i<msgheader.param1; ++i)
            {
              ...
              switch( msgheader.param2 )
              {
                case GLOBAL_TYPE_ANALOG:
				  ...
				  if (blDebugMode==2) printf("%f:%d", rbufA->value,rbufA[0].status);
				  if (blDebugMode==1)  { 
				      printf("%f:%d\n", rbufA->value,rbufA[0].status);
					  printf("tbl_id=%d\n", rbufA[0].tbl_id);
					  printf("id_channel=%d\n", rbufA[0].id_channel);
				   }
                   break;
			    case GLOBAL_TYPE_BOOL:
				  ...
				  if (blDebugMode==2) printf("%d:%d", rbufB->value,rbufB[0].status);
				  if (blDebugMode==1)  { 
				      printf("%f:%d\n", rbufB->value,rbufB[0].status);
					  printf("tbl_id=%d\n", rbufB[0].tbl_id);
					  printf("id_channel=%d\n", rbufB[0].id_channel);
				  }
                  break;
...
int SGmanualGetCanal(char *ip, int16_t portid, uint32_t gtype, uint32_t id, uint32_t tbl_id, int blDebugMode)
{
	int32_t err = 0 ;
	LOG_HEADER Logger;
    COMM_PORT *cp = NULL;
...
int SGmanualSet(char *ip, int16_t portid, uint32_t gtype, uint32_t id, uint32_t tbl_id, double val, char *login, char *pass, int id_channel, int blDebugMode)
{
	int ii = 0 ;
	int32_t err = 0 ;
	LOG_HEADER Logger;
	...
	DCP_ASTRUCT_MNL_COMMENT rpar[1];
	ACCESS_DATA access_data ;
	char comment[20]="External control\0" ;
	DCP_ASTRUCT_MNL_COMMENT *rbufAM = NULL ;
	DCP_BSTRUCT_MNL_COMMENT *rbufBM = NULL ;
    int32_t         key_time;
	char    password_value[MAX_PASSWD_LEN*4/3+10];
    char    key_value[RSDUCRYPTO_KEY_LEN];


	if (id_channel<0) id_channel=0;
    ...
    {
        msgheader.command = CMD_DCP_SETVAL;
        msgheader.status = DCP_OK;
        msgheader.time1970 = (uint32_t)RSDURTGUtils_Time70();
        msgheader.src_uid = 0xFFFF;
        msgheader.dst_uid = 0xFFFF;
        ...
        msgheader.param1 = 1;
        msgheader.param2 = gtype;
	    msgheader.data_len = sizeof(DCP_ASTRUCT_MNL_COMMENT)*1;	

		memset(&access_data, 0, sizeof(ACCESS_DATA));

		err = 0 ;
        key_time = time(0);
        if(RSDUCrypto_GetSessionKey(key_value, (int32_t)key_time)!= RSDUCRYPTO_OK)
        {
          err++;
          if (blDebugMode==1) printf(": RSDUCrypto_GetSessionKey error");
        }

        /* Шифруем пароль по ключу */
        /* Zolotov M.Yu. Шифруем также и последний, нулевой байт - 
             чтобы можно было выделить строку при ее расшифровке */
        memset(password_value, 0, MAX_PASSWD_LEN*4/3+10);
        if(RSDUCrypto_Encrypt(pass, password_value,
                               (uint32_t)strlen(pass)+1, key_value)!= RSDUCRYPTO_OK)
        {
          err++;
          if (blDebugMode==1) printf( ": RSDUCrypto_Encrypt error");
        }

	    for (ii=0;ii<(int)strlen(login);ii++ ) access_data.login[ii]=(signed char)login[ii];
	    if (err==0)
		    for (ii=0;ii<(int)strlen(password_value) ;ii++ ) access_data.password[ii] =(signed char)password_value[ii];
		else
			for (ii=0;ii<(int)strlen(pass) ;ii++ ) access_data.password[ii] =(signed char)pass[ii];

		for (ii=0;ii<(int)strlen(comment) ;ii++ ) rpar[0].comment[ii] =(signed char)comment[ii];
			
        rpar[0].tbl_id = 0; // tbl_id
		rpar[0].id = id;
        rpar[0].id_channel = id_channel;
		rpar[0].value = val;
        rpar[0].status = DCP_OK;
		rpar[0].access_data = access_data;  /* Данные пользователя для подключения к БД */

 
        rbufAM =(DCP_ASTRUCT_MNL_COMMENT *) RSDURTGUtils_SMalloc(1*sizeof(DCP_ASTRUCT_MNL_COMMENT));
        rbufBM =(DCP_BSTRUCT_MNL_COMMENT *) RSDURTGUtils_SMalloc(1*sizeof(DCP_BSTRUCT_MNL_COMMENT));		


        err = 0 ;
		if( RSDURTGUtils_SendCommPort(cp, &raddr, &msgheader, (unsigned char *)&rpar, COMM_PORT_TIMEOUT*100, &status, &Logger)!= E_OK ) {
			err++;
		} else {
             if(RSDURTGUtils_ReceiveCommPort(cp, &raddr, &msgheader, (unsigned char **)&rbufAM, COMM_PORT_TIMEOUT*100, &status, &Logger)!= E_OK ) {
               err++;
			 }
			// If no errors then show data from received buffer.
			if (err==0) {
			  for(uint32_t i=0; i<msgheader.param1; ++i)
              {
			    if (blDebugMode==0) printf("%d", msgheader.status);
				if (blDebugMode==1) printf("Status : %d\n", msgheader.status);
                if (blDebugMode==1) printf("Data length : %d\n", msgheader.data_len);
                switch( msgheader.param2 )
                {
                   case GLOBAL_TYPE_ANALOG:
				//	   //if (blDebugMode==0) printf("%f", rbufAM[0].value);
				//	   //if (blDebugMode==2) printf("%f:%d", rbufAM[0].value,rbufAM[0].status);
				//	   if (blDebugMode==1)  { 
				//		      printf("%f:%d\n", rbufAM[0].value,rbufAM[0].status);
				//			  printf("tbl_id=%d\n", rbufAM[0].tbl_id);
				//			  printf("id_channel=%d\n", rbufAM[0].id_channel);
				//	   }
                      break;
		...
		if (err!=0) printf("ERRSR");

        RSDURTGUtils_SFree(rbufAM);
		RSDURTGUtils_SFree(rbufBM);
    }
...
// argv[1]=ip argv[2]=port argv[3]=id argv[4]=admin argv[5]=passme argv[6]=val argv[7]=d|s   -- set
// argv[1]=ip argv[2]=port argv[3]=id argv[4]=d|s                                            -- get
static void show_usage(char* name)
{
    printf( "Usage: %s <option(s)>\n", name);
    printf( "Options:\n" );
    printf( "\tGET:ip port id\n" );
	printf( "\tSET:ip port id login pass value\n" );
...
int main(int argc, char* argv[])
{
  ...
  int mode_working = 0 ; // 0 get, 1 set
  uint16_t port_id = 0;
  char login[MAX_LOGIN_LEN]="\0";
  char pass[MAX_PASSWD_LEN]="\0";
...
//  }
  ...
  if (argc>=7) {
	strncpy(login, argv[4], MAX_LOGIN_LEN);
	strncpy(pass, argv[5], MAX_PASSWD_LEN);
    tmp_val = atof(argv[6]); 
	mode_working = 1;
  }
  ...
  if (mode_working == 0) {
    if (argc>=5) {
      if(strcmp(argv[4], (const char*)"-d")==0)
      {
        blDebugMode = 1;
      } 
	  if(strcmp(argv[4], (const char*)"-s")==0)
      {
        blDebugMode = 2;
      }
	}
  ...
  if (mode_working == 1) {
    if (argc>=8) {
      if(strcmp(argv[7], (const char*)"-d")==0)
      {
        blDebugMode = 1;
      } 
	  if(strcmp(argv[7], (const char*)"-s")==0)
      {
        blDebugMode = 2;
      }
	}
  ...
  if (blDebugMode==1) {
    printf("(%d)%s:%d gtype=%d id=%d tbl=%d val=%.8f login=%s pass=%s w=%d mode=%d\n",
		strlen(ip),ip,port_id,param2,id,tbl_id,tmp_val,login,pass,mode_working,blDebugMode);
  }

  if (mode_working == 0)
	  SGmanualGet( ip, port_id, param2, id, tbl_id, blDebugMode);
  if (mode_working == 1) {
	  int sd=SGmanualGetCanal(ip, port_id, param2, id, tbl_id, blDebugMode);
	  SGmanualSet( ip, port_id, param2, id, tbl_id, tmp_val, login, pass, sd, blDebugMode);
  }
