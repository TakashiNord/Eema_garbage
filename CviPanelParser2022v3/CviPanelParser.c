...
int cp1251_to_utf8( char * fi , char * fo )
{
  ...
  if(inf)
  {
    // пока не дойдем до конца, считываем по MAX_STRING_SIZE байт
                      ...
                      int attr_fr_thick, int attr_fr_color, int attr_bg_color, int zplane)
{
  ...
  fprintf(outfile ,"style: 'margin: 0; padding:0;;z-index: %d" , 0); // ,zplane  = 0 - важн порядок отрисовки панелей декора
