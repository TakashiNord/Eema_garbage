...
begin
  ...
  If Boolean(GetClassName(hWindow, buff, SizeOf( buff ))) then
    begin
      ...
      If pos(AnsiLowerCase('appbar'), s1) > 0 then
        begin
          // Showmessage(s);
...
begin
  ...
  If Boolean(GetClassName(WndCtrl, buff, SizeOf(buff))) then
    begin
      ...
      s1 := AnsiLowerCase(StrPas(buff));
      // вот тут ищем кусок класса  CVIRTLVDChild  или кусок по имени Панели
      If ((pos(AnsiLowerCase('appbar'), s) > 0)  and (pos(AnsiLowerCase('ok'), s1) > 0)) then
        begin
...
begin
   if Timer1.Enabled=True then  begin
      ...
      Button1.Caption:='Контролировать appbar';
   end
   else begin
      Timer1.Enabled:=true ;
      Button1.Caption:='Не Контролировать appbar';
   end;
...
begin
   SendMessage(WnD, WM_SYSCOMMAND, SC_SIZE, 1);
end;
