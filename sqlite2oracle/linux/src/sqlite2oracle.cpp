...
// Static variable
std::string   JobName = "SQLite to Oracle";
    ...
    for (auto &tb : TableNames)
    {
       ...
       if ( rc == SQLITE_OK )
       {
           while ( sqlite3_step( ppStmt ) == SQLITE_ROW )
           {
               ...
               TableNameCols.push_back( (const char *)sqlite3_column_text( ppStmt, 1 ) ); // название колонки
           }
       } else {
   ...
   */
    ...
    */
    ...
    if (create_index>0) {
      ...
      std::cout<<"\t\t - Увеличение размера файла (db-файла)!" << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    auto print_right_parameters = [](std::string program)
    {
        ...
        std::cout << "\tTNSDB          - TNS - RSDU database\n";
        std::cout << "\nexamples:\n";
        std::cout << "\t to Oracle:\n";
...
// получение имен таблиц в файле
 ...
 // обход всех таблиц в sqlite БД
    for (auto &tb : TableNames)
    {
       ...
       std::string QCol = "" ;
       std::string QColPunc = "," ;
       int QColNum[]={0,0,0,0,0,0,0,0}; // массив заведен для вывода и быстроты доступа (чтобы каждый раз не искать в векторе)
       ...
       {  QCol="ID"; QColNum[0]=1; }
          ...
          QCol="0";
       if ( std::find(TableNameCols.begin(), TableNameCols.end(), "TIME1970") != TableNameCols.end() )
       {  QCol=QCol+QColPunc+"TIME1970"; QColNum[1]=1; }
       else
          QCol=QCol+QColPunc+"0";
       if ( std::find(TableNameCols.begin(), TableNameCols.end(), "TIME_MKS") != TableNameCols.end() )
       {  QCol=QCol+QColPunc+"TIME_MKS"; QColNum[2]=1; }
       else
          QCol=QCol+QColPunc+"0";
       if ( std::find(TableNameCols.begin(), TableNameCols.end(), "STATE") != TableNameCols.end() )
       {  QCol=QCol+QColPunc+"STATE"; QColNum[3]=1; }
       else
          QCol=QCol+QColPunc+"0";
       if ( std::find(TableNameCols.begin(), TableNameCols.end(), "VAL") != TableNameCols.end() )
       {  QCol=QCol+QColPunc+"VAL"; QColNum[4]=1; }
       else
          QCol=QCol+QColPunc+"0";
       if ( std::find(TableNameCols.begin(), TableNameCols.end(), "MIN_VAL") != TableNameCols.end() )
       {  QCol=QCol+QColPunc+"MIN_VAL"; QColNum[5]=1; }
       else
          QCol=QCol+QColPunc+"0";
       if ( std::find(TableNameCols.begin(), TableNameCols.end(), "MAX_VAL") != TableNameCols.end() )
       {  QCol=QCol+QColPunc+"MAX_VAL"; QColNum[6]=1; }
       else
          QCol=QCol+QColPunc+"0";
       ...
       log.Info("Вызов процедуры очистки таблицы стэка (arc_writer_pkg.prepare_table)..., db_err = %d", db_err);
       if (db_err == 0)
       {
            std::cout << "Вызов процедуры очистки таблицы стэка (arc_writer_pkg.prepare_table)..." << std::endl;
            sql[0]='\0';
            sprintf(sql, "declare vRetVal VARCHAR2 (1024); begin vRetVal:= arc_writer_pkg.prepare_table( '%s' ); end;", tb.c_str() );
            int cn = RSDURTGUtils_DBSQLExec(ui, sql, DBTimeout, log.getPointer()) ;
            if (cn < 0)
            {
                std::cerr << "prepare_table: <" << sql << ">, Error message: " << RSDURTGUtils_DBGetError(ui) << std::endl;
                ++db_err;
                //RSDURTGUtils_LogTimeOff(log.getPointer());
                //RSDURTGUtils_UnilogMessage(LOG_ERR, log.getPointer(), "%s", sql);
                //RSDURTGUtils_LogTimeOn(log.getPointer());
            }
            else
            {

                char *rec = NULL;
                while( (rec = RSDURTGUtils_DBFetch(ui) ) != NULL )
                {
                    if (strlen(rec)>1)
                    {
                        std::cout << "prepare_table: " << rec << std::endl;
                        ++db_err;
                        log.Info("arc_writer_pkg.prepare_table = %s", rec);
                    }
                 }
       ...
       std::cout << "Вызов процедуры очистки таблицы стэка (arc_writer_pkg.prepare_table)...\e[1;31m Ok \e[0m" << std::endl;


       //////////////////////////////////////////////////////////////////////////
       //Перенос архивов в Oracle
       log.Info("Starting Data reading and loading to Oracle");
           ...
           for (const auto &profile : listProfiles)
           {
               ...
               if ( ReturnCode == SQLITE_OK )
               {
                  ...
                  if (bParamRange) {
                   ...
                   std::cout <<  "Prepare to exec = " << sqlite3_errmsg(db_handle) << std::endl;
                   sqlite3_finalize( ppStmt );
                   continue ; // следующий профиль
               }


               int processedArchive = 0 ; // подсчет числа вставленных записей

               int ncolumns = sqlite3_column_count(ppStmt); // число колонок в запросе
               std::cout << "A("<<" col="<<ncolumns<<")"<< std::flush;
               ...
               while ( 1 ) // sqlite3_step( ppStmt ) == SQLITE_ROW       ---- SQLITE_DONE
               {
                   ...
                   if (step_flag==SQLITE_ROW) {
                       ...
                       unsigned id = 0, time1970 = 0, time_mks = 0, state = 0;
                       double   val = 0, min_val = 0, max_val = 0;
                       db_err = 0;
                       ...
                       if (GLOBAL_TYPE_ANALOG == profile.GtypeID)
                       {
                           ...
                           case SQLITE_NULL:
                               ...
                               break;
                           default :
                               val = sqlite3_column_double(ppStmt, 4);
                               break;
                           }
                       }
                       else if(GLOBAL_TYPE_BOOL == profile.GtypeID)
                       {
                           ...
                           case SQLITE_NULL:
                               ...
                               break;
                           default :
                               val = sqlite3_column_int(ppStmt, 4);
                               break;
                           }
                       }
                       min_val = sqlite3_column_double(ppStmt, 5);
                       ...
                       //std::cout << "(" << processedArchive << ")= " << id << " , " << time1970 << " , " << time_mks << " , "
                       //            << state << " , " << val << " , " << min_val << " , " << max_val << std::endl;
                       ...
                       if ( QColNum[0]==1 ) {  values=std::to_string(id);                       qcol="ID"; }
                       if ( QColNum[1]==1 ) {  values=values+QColPunc+std::to_string(time1970); qcol=qcol+QColPunc+"TIME1970"; }
                       if ( QColNum[2]==1 ) {  values=values+QColPunc+std::to_string(time_mks); qcol=qcol+QColPunc+"TIME_MKS"; }
                       if ( QColNum[3]==1 ) {  values=values+QColPunc+std::to_string(state);    qcol=qcol+QColPunc+"STATE"; }
                       if ( QColNum[4]==1 ) {  values=values+QColPunc+std::to_string(val);      qcol=qcol+QColPunc+"VAL"; }
                       if ( QColNum[5]==1 ) {  values=values+QColPunc+std::to_string(min_val);  qcol=qcol+QColPunc+"MIN_VAL"; }
                       if ( QColNum[6]==1 ) {  values=values+QColPunc+std::to_string(max_val);  qcol=qcol+QColPunc+"MAX_VAL"; }

                       // INSERT to tb.c_str()
                       std::string sqlout = "INSERT INTO " + tb + " ( " + qcol + " ) values ( " + values + " )" ;
                       sql[0]='\0';
                       sprintf(sql, "INSERT INTO %s ( %s ) values ( %s ) ", tb.c_str(), qcol.c_str(), values.c_str() );

                       //std::cout << sql << std::endl;

                       if (RSDURTGUtils_DBSQLExec(ui, sql, DBTimeout, log.getPointer()) < 0)
                       {
                           RSDURTGUtils_UnilogMessage(LOG_ERR, log.getPointer(), "Main task: %s, %s", sql, RSDURTGUtils_DBGetError(ui));
                       }
                       else
                       {
                           ++processedArchive;

                           //// нет , необходимости выполнять Commit после каждого INSERT
                           //if (RSDURTGUtils_DBSQLExec(ui, "COMMIT", 100, log.getPointer()) < 0)
                           //{
                           //    RSDURTGUtils_UnilogMessage(LOG_ERR, log.getPointer(), "Main task: COMMIT failed %s", RSDURTGUtils_DBGetError(ui));
                           //}
                       }
                       RSDURTGUtils_DBDataDispose(ui);
                   ...
                   if (processedArchive % 10000 == 0)
                       std::cout << "." << std::flush;

               } // while
               sqlite3_finalize( ppStmt );

               std::cout << "Insert records = " << processedArchive << std::endl;

               if (processedArchive>0) {
                 ...
                 log.Info("Вызываем перенос (arc_writer_pkg.move_data) из таблицы стэка..., db inserts = %d", processedArchive);

                 std::cout << std::endl;
                 std::cout << "Вызываем перенос (arc_writer_pkg.move_data) из таблицы стэка..." << std::endl;
                 //std::cout << "Число записей = " << processedArchive << std::endl;
                 sql[0]='\0';
                 sprintf(sql, "declare vRetVal VARCHAR2 (1024); begin vRetVal:= arc_writer_pkg.move_data( '%s' ); end;", tb.c_str() );
                 int cn = RSDURTGUtils_DBSQLExec(ui, sql, DBTimeout, log.getPointer()) ;
                 if (cn < 0)
                 {
                     std::cerr << "move_data: <" << sql << ">, Error message: " << RSDURTGUtils_DBGetError(ui) << std::endl;
                     ++db_err;
                 }
                 else
                 {
                     ...
                     while( (rec = RSDURTGUtils_DBFetch(ui) ) != NULL )
                     {
                         if (strlen(rec)>1)
                         {
                             std::cout << "move_data: " << rec << std::endl;
                             ++db_err;
                             log.Info("arc_writer_pkg.move_data = %s", rec);
                         }
                      }
                 ...
                 if (db_err==0) {
                   std::cout << "Вызов переноса (arc_writer_pkg.move_data) из таблицы стэка...\e[1;31m Ok \e[0m" << std::endl;
                 }
       ...
       {
           std::cerr << "Load: Standard exception : " << e.what() << std::endl;
       }
       log.Info("End loading to Oracle");
    ...
    std::cout << std::endl;
    std::cout << "Procedure loading to Oracle \e[1;31m finished \e[0m" << std::endl;
    std::cout << std::endl;
