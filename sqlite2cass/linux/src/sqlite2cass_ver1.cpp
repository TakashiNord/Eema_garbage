...
// получение имен таблиц в файле
    ...
    for (auto &tb : TableNames)
    {
       ...
       sizeFile = 0 ;
       rows = 0;
       strpartfile[0]='\0';sprintf(strpartfile,"%03d",partfile);
...
*/
           ...
           while ( sqlite3_step( ppStmt ) == SQLITE_ROW ) // SQLITE_DONE
           {
               ...
               if ( (sizeFile/(1024*1024)) > 1024 ) // > 1024 mb
               {
                 outFile.flush();
                 outFile.close();
...
// получение имен таблиц в файле
    ...
    for (auto &tb : TableNames)
    {
       ...
       //  select  TIME1970,TIME_MKS,STATE,VAL,MIN_VAL,MAX_VAL
       std::string QCol = "" ;
       std::string QColPunc = "," ;
       ...
       std::string sql0 = "SELECT " + QCol + " FROM " + tb ;
       std::string sql10 = "SELECT " + QCol + " FROM " + tb + " WHERE TIME1970>= ? and TIME1970<= ? and ID= ? ; " ;
       std::string sql1 = "SELECT " + QCol + " FROM " + tb + " WHERE time1970 between ? and ? and ID=? ; " ;
       std::string sql2 = "SELECT " + QCol + " FROM " + tb + " WHERE TIME1970>= ? and TIME1970<= ? and ID>= ? and ID<= ? ;" ;
       ...
       //Чтение списка соответствующих таблиц из %_ARC
       if (0 == db_err)
       {
           std::cout << "Reading Archive tables list:" << std::endl;
           for (auto &profile : listProfiles)
           {
               if (bParamRange)
                   sprintf(sql, SQLSelectArcTablesRangeParamsSQLITE, profile.ArcTblName, ID_profile, start_id_param, end_id_param);
               else
                   sprintf(sql, SQLSelectArcTablesSQLITE, profile.ArcTblName, ID_profile );
               int cn = RSDURTGUtils_DBSQLExec(ui, sql, DBTimeout, log.getPointer());
               if (cn > 0)
               {
                   char *psz = NULL;
                   while ((psz = RSDURTGUtils_DBFetch(ui)) != NULL)
                   {
                       ...
                       if (sscanf((const char *)psz, SQLSelectArcTablesFMT, &archive.ParamID, archive.TblName) != SQLSelectArcTablesNUM)
                       {
                           std::cerr << "Error scan DB row" << std::endl;
                           ++db_err;
                       }
                       ...
                       {
                           rtrim(archive.TblName, sizeof(archive.TblName));
                           profile.listArchives.push_back(archive);
                           //std::cout << "Archive ParamID " << archive.ParamID << " TblName: " << archive.TblName << std::endl;
                       }
                   }
               }
               else
               ...
               RSDURTGUtils_DBDataDispose(ui);
               std::cout << "Read TOTAL for ( " << profile.LstTblID << " , " << ID_profile << " ) : " << profile.listArchives.size() << " Archives \n" << std::endl;
               totalArchives += profile.listArchives.size();
           }
       }
       ...
       try
       {
           ...
           //Выборка данных по списку таблиц согласно заданному диапазону
           for (const auto &profile : listProfiles)
           {
               ...
               for (const auto &archive : profile.listArchives)
               {
                   ++processedArchive;
                   double processedPercent = 100.0 * processedArchive / totalArchives;
                   auto currentLoad = std::chrono::high_resolution_clock::now();
                   std::chrono::duration<double> currentElapsed = currentLoad - beginLoad;
                   if (processedPercent < 1e-10)
                       processedPercent = 1e-10;
                   double estimateLoad = (currentElapsed.count() / processedPercent) * (100.0 - processedPercent);
                   unsigned estimateLoadU = static_cast<unsigned>(std::ceil(estimateLoad));
                   std::stringstream estimateLoadstr, passedLoadstr;
                   estimateLoadstr << std::setfill('0') << std::setw(2) <<  estimateLoadU / 3600 << ":"
                                   << std::setfill('0') << std::setw(2) << (estimateLoadU % 3600) / 60 << ":"
                                   << std::setfill('0') << std::setw(2) <<  estimateLoadU % 60;
                   unsigned currentElapsedU = static_cast<unsigned>(std::floor(currentElapsed.count()));
                   passedLoadstr << std::setfill('0') << std::setw(2) << currentElapsedU / 3600 << ":"
                                 << std::setfill('0') << std::setw(2) << (currentElapsedU % 3600) / 60 << ":"
                                 << std::setfill('0') << std::setw(2) << currentElapsedU % 60;

                   std::cout << "Proceed archive " << std::setfill(' ') << std::setw(15) << archive.ParamID << " [" << processedArchive << "/" << totalArchives <<"] ["
                             << std::fixed << std::setprecision(2) << processedPercent
                             << "%] [time estimated left/passed: " << estimateLoadstr.str() << "/" << passedLoadstr.str() << "]: " << "selecting..." << std::flush;
                   //Прогресс
                   unsigned progressDot = 0;
...
//============================================================================
                   ...
                   time_t tSplitDateFrom = input_start_time;
                   time_t tSplitDateTo = tSplitDateFrom + (profile.Interval ? profile.Interval : 1) * MaxDataCountinResult;
                   time_t tDateTo = input_end_time;
                   ...
                   while (tSplitDateFrom <= tDateTo)
                   {
                       ...
                       //std::cout << "Prepare to exec <" << sql1 << ">..." << std::endl;
                       std::cout << "R" << std::flush;

                       std::string sql00 = sql0 + " WHERE TIME1970 between " + std::to_string(tSplitDateFrom)
                                                + " and " + std::to_string(tSplitDateTo) + " and ID=" + std::to_string(archive.ParamID) ;

                       ReturnCode = sqlite3_prepare_v2( db_handle, sql00.c_str() , -1, &ppStmt, NULL );
                       if ( ReturnCode == SQLITE_OK )
                       {
                          sqlite3_reset(ppStmt);
                          sqlite3_clear_bindings(ppStmt);
                        /*
                          ReturnCode = sqlite3_bind_int64(ppStmt, 1, tSplitDateFrom);
                          if( ReturnCode != SQLITE_OK ) {
                            std::cout << " sqlite3_bind_int error = " << sqlite3_errmsg(db_handle) << std::endl;
                            break;
                          }
                          ReturnCode = sqlite3_bind_int64(ppStmt, 2, tSplitDateTo);
                          if( ReturnCode != SQLITE_OK ) {
                            std::cout << " sqlite3_bind_int error = " << sqlite3_errmsg(db_handle) << std::endl;
                            break;
                          }
                          ReturnCode = sqlite3_bind_int(ppStmt, 3, archive.ParamID);
                          if( ReturnCode != SQLITE_OK ) {
                            std::cout << " sqlite3_bind_int error = " << sqlite3_errmsg(db_handle) << std::endl;
                            break;
                          }
                       ...
                       int cn = (int)(tSplitDateTo-tSplitDateFrom)/(profile.Interval ? profile.Interval : 1) ;
                       cn = abs(cn) + 2 ;

                       std::cout << "A" << std::flush;
                       //std::cout << "...executed, result = " << cn << std::endl;
                       if (cn > 0)
                       {
                           int flag_cn = 0;
                           oracle_archive.reserve(cn); //- перемещено в цикл
                           ...
                           {
                               //if (flag_cn <= 0) {
                               //  oracle_archive.reserve(cn);
                               //  flag_cn = 1 ;
                               //}

                               ARC_STRUCT arcstruct;
                               unsigned time1970 = 0, time_mks = 0, state = 0;
                               unsigned bval = 0;
                               double   aval = 0, min_val = 0, max_val = 0;
                               db_err = 0;
...
/*
                               for (int i = 0; i < ncolumns; i++) {
                                   ...
                                   switch (type) {
                                   case SQLITE_INTEGER:
                                       printf("%lld", sqlite3_column_int64(ppStmt, i));
                                       break;
                                   case SQLITE_FLOAT:
...
*/
                               ...
                               time1970 = sqlite3_column_int64(ppStmt, 0);
                               time_mks = sqlite3_column_int(ppStmt, 1);
                               state = sqlite3_column_int64(ppStmt, 2);
                               if (GLOBAL_TYPE_ANALOG == profile.GtypeID)
                               {
                                   aval = sqlite3_column_double(ppStmt, 3);
                                   arcstruct.val = aval;
                               }
                               else if(GLOBAL_TYPE_BOOL == profile.GtypeID)
                               {
                                   bval = sqlite3_column_int(ppStmt, 3);
                                   arcstruct.val = bval;
                               }
                               min_val = sqlite3_column_double(ppStmt, 4);
                               max_val = sqlite3_column_double(ppStmt, 5);

                               //std::cout << ".. " << time1970 << " , " << time_mks << " , " << state << " , "
                               //          << arcstruct.val << " , " << min_val << " , " << max_val << std::endl;

                               if (0 == db_err)
                               {
                                   arcstruct.id_tbllst = static_cast<int32_t>(profile.LstTblID);
                                   arcstruct.id_ginfo = static_cast<int32_t>(ID_profile);
                                   arcstruct.id = static_cast<int32_t>(archive.ParamID);
                       ...
                       // Calculate RestoreDate from and to.
                       tSplitDateFrom = tSplitDateTo + 1;
                       tSplitDateTo = tSplitDateFrom + (profile.Interval ? profile.Interval : 1) * MaxDataCountinResult;
                   }// while (tSplitDateFrom <= tDateTo)
...
//==   ==========================================================================
                   ...
                   while (toSplitEnd < NumRecords && num_errs < 5 /* max 2 attemps */)
                   {
