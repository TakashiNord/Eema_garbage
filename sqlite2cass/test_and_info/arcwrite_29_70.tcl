...
# файл SQLite
 set sqlname "D:\\MEAS_ARC_29_70.db"

# Запросы SQLite
# с 27 по 29 мая
# set strSQL1 "SELECT ID,TIME1970,VAL,STATE FROM MEAS_ARC_29_70 WHERE TIME1970>=1685125590 AND TIME1970<=1685471190 ORDER BY ID asc"
  set strSQL1 "SELECT ID,TIME1970,VAL,STATE FROM MEAS_ARC_29_70 WHERE TIME1970>=1685525190 AND TIME1970<=1686043590 AND ID>=524288 ORDER BY ID asc"
# 01 по 05 июня
# set strSQL1 "SELECT ID,TIME1970,VAL,STATE FROM MEAS_ARC_29_70 WHERE TIME1970>=1685525190 AND TIME1970<=1686043590 ORDER BY ID asc"
# set strSQL1 "SELECT ID,TIME1970,VAL,STATE FROM MEAS_ARC_29_70 WHERE TIME1970>=1685525190 AND TIME1970<=1686043590 AND ID=%s ORDER BY ID asc"

# время хранения в часах профиля в Oracle
 set profile_Depth 43848 ;

# шаблон строки для вставки в Cassandra
 set sout "INSERT INTO arc.profile_70 (id_tbllst,id,time1970,max_val,min_val,state,val) values (29, %s , %s , 0.0 , 0.0 , %s , %s ) IF NOT EXISTS USING TTL %s ;"
...
#  set sqlstrR "select ELREG_LIST_V.id as id from MEAS_SNAPSHOT30_TUNE , ELREG_LIST_V where MEAS_SNAPSHOT30_TUNE.ID=ELREG_LIST_V.id order by id asc"
#  set r33 [ db2 $sqlstrR ]
#  puts "id cnt=[llength $r33]"
...
#foreach {r3} $r33 {
#  set iddb [lindex $r3 0]
#  set s2s [ format $strSQL11  $iddb ]
