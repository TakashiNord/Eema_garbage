...
def smreceivet(s, slen=1024):
    data = ""
    LTIMEOUT = 15
    #s.setblocking(1)

    inout = [s]
    infds, outfds, errfds = select.select(inout, inout, [], LTIMEOUT)
    if len(infds) != 0:
        try:
            data = s.recv(slen)
    ...
    #bytes_recd = 0
    #while bytes_recd < slen:
    #    infds, outfds, errfds = select.select(inout, inout, [], LTIMEOUT)
    #    if len(infds) != 0:
    #        chunk = s.recv(min(slen - bytes_recd, 1024))
    #        if not chunk:
    #            pass
    #            break
...
def smonreceivet(s, slen):
    # command, src_uid, dst_uid, param1, param2, param3, laws, key, data_len, status, time1970
    ret = (0, 0, 0, 0, 0, 0, 0, b'\0', 0, 0, 0)
    fm = 'IIIIIII16sIhI'
    fm1 = uh(fm)
    ln1 = struct.calcsize(fm1)

    dt = smreceivet(s, ln1)
    if len(dt) <=0: return dt

    try:
        fd = struct.unpack(fm1, dt)
    except Exception as e:
        fd = ret
    finally:
        pass
    cm0 = int(fd[0])
    if cm0 > 65535 : cm0 = int(unpackVal(fd[0], 'I'))
    st0 = int(fd[9])
    if st0 > 65535 : st0 = int(unpackVal(fd[9], 'I'))
    #print('fd=',fd)
    if int(GCMD_DATASERVICEINIT) == cm0 :
        if st0 == 0 :
            time.sleep(1)
            dt = smreceivet(s, slen)
            pass
...
if __name__ == '__main__':
    ...
    print(f'Command ({GCMD_GLOBAL_STATE_STOPPED},{GCMD_GLOBAL_STATE_MASTER},{GCMD_GLOBAL_STATE_SLAVE})= {cm1}')
    pass
