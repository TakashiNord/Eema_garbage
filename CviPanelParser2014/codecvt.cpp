...
{
	int max_input = (int)(from_end - from) & ~1;
	int max_output = (int)(to_limit - to);
...
{
	int max_input = (int)(from_end - from);
	int max_output = (int)(to_limit - to) & ~1;
					   ...
					   wchar_t* to, wchar_t* to_limit, wchar_t*& to_next) const
{
	...
	for(; to_next < to_limit && from_next < from_end; ++to_next) {
		...
		if (uchar(*from_next) < 0x80) *to_next = uchar(*from_next++);
			...
			int zero_bit_pos = (int)most_signifant_bit_position(~*from_next);
